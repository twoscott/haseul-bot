...
exports.onCommand = async function(message, args) {
    ...
    case 'say':
        if (checkPermissions(member, ['MANAGE_MESSAGES'])) {
            channel.send({ content: '⚠ Command deprecated. Please use `.message send`/`.msg send` instead.' });
        }
        break;
    case 'edit':
        if (checkPermissions(member, ['MANAGE_MESSAGES'])) {
            channel.send({ content: '⚠ Command deprecated. Please use `.message edit`/`.msg edit` instead.' });
        }
        break;
    case 'get':
        if (checkPermissions(member, ['MANAGE_MESSAGES'])) {
            channel.send({ content: '⚠ Command deprecated. Please use `.message get`/`.msg get` instead.' });
        }
        break;
    case 'poll':
        switch (args[1]) {
...
async function sendMessage(message, args) {
    ...
    if (args.length < 3) {
        message.channel.send({ content: '⚠ No channel provided to send a message to.' });
        return;
    }

    const channelID = parseChannelID(args[2]);
    if (!channelID) {
        message.channel.send({ content: '⚠ Please provide a channel to send the message to.' });
        return;
    }

    const channel = guild.channels.cache.get(channelID);
    if (!channel) {
        message.channel.send({ content: '⚠ Invalid channel provided or channel is not in this server.' });
        return;
    }
...
async function editMessage(message, args) {
    ...
    if (args.length < 3) {
        message.channel.send({ content: '⚠ No message url provided.' });
        return;
    }

    const messageUrlMatch = args[2].match(messageUrlRegex);
    if (!messageUrlMatch) {
        message.channel.send({ content: '⚠ Invalid message url provided.' });
        return;
    }

    const [guildID, channelID, messageID] = messageUrlMatch.slice(1);
    if (guildID != guild.id) {
        message.channel.send({ content: '⚠ Provided message url must link to a message in this server.' });
        return;
    }

    const channel = guild.channels.cache.get(channelID);
    if (!channel) {
        message.channel.send({ content: '⚠ Provided message url links to an invalid channel.' });
        return;
    }
    ...
    if (!msg) {
        message.channel.send({ content: '⚠ Provided message url links to an invalid message.' });
        return;
    }

    if (msg.author.id != Client.user.id) {
        message.channel.send({ content: '⚠ Provided message url links to a message sent by another user.' });
        return;
    }

    if (args.length < 4) {
        message.channel.send({ content: '⚠ No content provided to edit the message with.' });
        return;
    }

    const content = trimArgs(args, 3, message.content);
    await msg.edit(content);
    message.channel.send({ content: `Message edited in <#${channel.id}>.` });
}
...
async function getMessage(message, messageUrl) {
    ...
    if (msg.content.length < 1) {
        message.channel.send({ content: '⚠ Message has no content.' });
        return;
    }
...
async function togglePoll(message) {
    ...
    message.channel.send({ content: `Poll setting turned ${tog ? 'on':'off'}.` });
}
...
async function setPrefix(message, prefix) {
    ...
    if (!prefix) {
        message.channel.send({ content: '⚠ Please provide a prefix for commands to use.' });
        return;
    }
    if (prefix.length > 1) {
        message.channel.send({ content: '⚠ A prefix must be a single character.' });
        return;
    }
    if (prefix.match(/^\w+$/)) {
        message.channel.send({ content: '⚠ A prefix cannot be a letter.' });
        return;
    }

    serverSettings.set(guild.id, 'prefix', prefix);
    message.channel.send({ content: `Prefix set to \`${prefix}\`` });
}
