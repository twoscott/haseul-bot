...
exports.toggle = async function(col) {
    ...
    if (statement.changes) {
        let row = await db.get(`SELECT ${col} FROM clientSettings WHERE clientID = ?`, [Client.user.id]);
...
exports.getSettings = async function() {
    ...
    let row = await db.get(SQL`SELECT * FROM clientSettings WHERE clientID = ${Client.user.id}`);
    return row;
}