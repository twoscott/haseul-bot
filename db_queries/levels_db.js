...
const dbopen = sqlite.open('./haseul_data/levels.db');
...
//     await db.run(`CREATE TABLE IF NOT EXISTS globalXpNew(
//         userID TEXT NOT NULL PRIMARY KEY,
//         xp INT NOT NULL DEFAULT 0
//     )`);
//     let rows = await db.all(SQL`SELECT * FROM globalXp`);
//     for (let row of rows) {
//         await db.run(SQL`INSERT OR IGNORE INTO globalXpNew VALUES (${row.userID}, ${row.xp})`);
//     }
//     await db.run(SQL`DROP TABLE globalXp`);
//     await db.run(SQL`ALTER TABLE globalXpNew RENAME TO globalXp`);

//     await db.run(SQL`DROP TABLE exclusions`);
//     console.log("Finished altering levels.db");
// })
