...
exports.scrapeArtistImage = async function(artist) {
    ...
    const images = doc.getElementsByClassName('image-list-item-wrapper');
    if (images.length < 1) {
...
exports.scrapeArtistsWithImages = async function(
    username, datePreset, itemCount) {
    const collection = [];
    const pageCount = Math.ceil(itemCount / 50);
    ...
    for (let i = 0; i < pageCount; i++) {
        ...
        const doc = new JSDOM(response.data).window.document;
        const rows = doc.getElementsByClassName('chartlist-row link-block-basic js-link-block');
...
exports.getTimeFrame = function(timeframe) {
    ...
    const week = ['7', '7day', '7days', 'weekly', 'week', '1week'];
    const month = ['30', '30day', '30days', 'monthly', 'month', '1month'];
    const threeMonth = ['90', '90day', '90days', '3months', '3month'];
    const sixMonth = ['180', '180day', '180days', '6months', '6month'];
    const year = ['365', '365day', '365days', '1year', 'year', 'yr', '12months', '12month', 'yearly'];
    const overall = ['all', 'at', 'alltime', 'forever', 'overall'];
